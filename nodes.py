...
class Nodes:
    def __init__(self, qbits, place_qbits_in_lines: bool = True, coords=None):
        ...
        node_objects = [
            Node(n, coord) for n, coord in zip(self.qbits.graph.nodes, coords)
        ]
    ...
    def set_qbits_of_nodes(self):
        ...
        for qbit in self.qbits:
            qbit.length_of_its_nodes = tuple(
                [self.length_of_nodes.get(node) for node in qbit.qubit]
            )
    ...
    def remove_ancillas_from_nodes(self, ancillas):
        ...
        for ancilla in ancillas:
            for node_name, node_object in self.nodes.items():
                if node_name in ancilla:
                    node_object.qbits = [
                        qbit for qbit in node_object.qbits if qbit.qubit != ancilla
                    ]
    ...
    def nodes_of_qubit(self, qbit):
        ...
        if self.nodes[qbit.qubit[0]].coord > self.nodes[qbit.qubit[1]].coord:
            qbit.coord = (
                self.nodes[qbit.qubit[1]].coord,
                self.nodes[qbit.qubit[0]].coord,
            )
        else:
            qbit.coord = (
                self.nodes[qbit.qubit[0]].coord,
    ...
    def propose_coords(self):
        ...
        coords_for_ancillas = list(
            set(
                [
                    coord
                    for coords in qbit_coords
    ...
    def propose_ancillas(self, allowed_number):
        ...
        for coord in coords_for_ancillas:
            number_of_new_plaqs = Nodes.number_of_plaquettes_of_ancilla(
                qbit_coords, coord
            )
            # if below 2, this ancilla makes no sense
            if number_of_new_plaqs > 1:
                ancillas.append(
                    (number_of_new_plaqs, self.name_of_ancilla(coord), coord)
                )
        ancillas.sort(reverse=True)
        best_ancillas = {
            ancilla: coord
            for number_of_new_plaqs, ancilla, coord in ancillas[:allowed_number]
            if number_of_new_plaqs == 4
        }
        if len(best_ancillas) == 0:
            best_ancillas = {ancilla: coord for _, ancilla, coord in ancillas[:1]}
    ...
    def propose_ancillas_to_remove(self):
        ...
        *** or not involved in plaquettes
        ...
        found_plaquettes = self.qbits.found_plaquettes_around_coords(
            coords_around_ancillas
        )
        ...
        ancillas_to_keep = [
            ancilla
            for ancilla in ancillas_in_plaquettes
